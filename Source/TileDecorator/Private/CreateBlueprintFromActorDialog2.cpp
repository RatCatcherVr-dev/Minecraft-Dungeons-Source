...
#include "CreateBlueprintFromActorDialog2.h"
...
TWeakObjectPtr<AActor> FCreateBlueprintFromActorDialog2::ActorOverride;

void FCreateBlueprintFromActorDialog2::OpenDialog(bool bInHarvest, AActor* InActorOverride )
{
	...
	(
		...
		.OnCreateAssetAction(FOnPathChosen::CreateStatic(FCreateBlueprintFromActorDialog2::OnCreateBlueprint, bInHarvest))
	);
...
void FCreateBlueprintFromActorDialog2::OnCreateBlueprint(const FString& InAssetPath, bool bInHarvest)
{
	...
	if(bInHarvest)
	{
		...
		Blueprint = FKismetEditorUtilities::HarvestBlueprintFromActors(InAssetPath, Actors, false);
	}
	else
	{
		...
		const bool bReplaceActor = false;
