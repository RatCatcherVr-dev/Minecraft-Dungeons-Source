...
BehaviorTuple createSheepRed(AMobCharacter& mob, const UBehaviorOptionsComponent& options) {
	static Random rnd;

	const auto rightHand = rnd.nextBoolean() ? -1.f : 1.f;	

	const auto isPetTarget = [](AActor* actor) -> bool {
	...
	return BehaviorTuple(
		root(selector("SheepRed",
			uninterruptible(sequence(
				startPredicate(locator::IsInRange(actor::Master(), { options.Get("AttackTriggerDistance", 1000.f) })),
