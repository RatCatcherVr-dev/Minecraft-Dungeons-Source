...
bool isHostileFrostwarden(AActor* actor) {
	const auto mobCharacter = Cast<AMobCharacter>(actor);

	return mobCharacter &&
		mobCharacter->EntityType != EntityType::GeomancerAncient &&
...
BehaviorTuple createFrostWarden(AMobCharacter& mob, const UBehaviorOptionsComponent& options) { return BehaviorTuple(
	root(selector("FrostWarden",
		...
		uninterruptible(sequence("full-offensive-state",
			startPredicate(
				...
				lessThan(actor::CountInRange(actor::Self(), 3000.f, isHostileFrostwarden), value(3))
			),
			behavior::facing(
				make_unique<USummonFormationTask>(
					...
					0.s,
					0.s,
					formation::FrostWardenBombMultiple
				)
			)
		)),
