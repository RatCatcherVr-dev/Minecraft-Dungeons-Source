...
#if defined(WEBRTC_POSIX) || defined(WEBRTC_SWITCH) || defined(WEBRTC_ORBIS)
typedef int SOCKET;
#endif  // WEBRTC_POSIX
...
namespace rtc {
...
#elif defined(WEBRTC_POSIX) || defined(WEBRTC_SWITCH) || defined(WEBRTC_ORBIS)
		virtual int GetDescriptor() = 0;
		virtual bool IsDescriptorClosed() = 0;
#endif
#if defined(WEBRTC_ORBIS)
		virtual void AddEventListener(SceKernelEqueue equeue) = 0;
		virtual void RemoveEventListener(SceKernelEqueue equeue) = 0;
#endif
	};
	...
	public:
		...
		bool SwitchWait(int cms, Dispatcher *SignalDispatcher);
#endif
...
#if defined (WEBRTC_ORBIS)
		const SceKernelEqueue GetEventQueue() { return event_queue; }
		const int GetNextDescriptor() { return ++next_descriptor_; }
#endif
	...
	int next_descriptor_ = 1000;
#endif

#if defined(WEBRTC_POSIX) || defined(WEBRTC_SWITCH)
	...
	bool WaitSelectOrbis(int cmsWait, bool process_io);
#endif  // WEBRTC_POSIX
...
#if defined(WEBRTC_ORBIS)
		SceNetId epoll_fd_ = INVALID_SOCKET;
#else
		int epoll_fd_ = INVALID_SOCKET;
#endif
#if defined(WEBRTC_ORBIS)
		std::vector<SceNetEpollEvent> epoll_events_;
#else
...
#endif
#if defined(WEBRTC_ORBIS)
		virtual void AddEventListener(SceKernelEqueue equeue) override;
		virtual void RemoveEventListener(SceKernelEqueue equeue) override;
#endif
	private:
#if defined(WEBRTC_WIN)
		static int next_id_;
