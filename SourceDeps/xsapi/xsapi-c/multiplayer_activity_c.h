...
{
    /// <summary>
    /// Default. This has no inherent meaning - it is up to the title to interpret this value appropriately.
    /// </summary>
    Default = 0,
...
{
    /// <summary>Xuid of the encountered user.</summary>
    uint64_t xuid;

    /// <summary>The type of encounter.</summary>
    XblMultiplayerActivityEncounterType encounterType;
} XblMultiplayerActivityRecentPlayerUpdate;

/// <summary>
/// Appends to a user's list of recent players. If an encountered user is already in the list, it updates the 
/// existing recent player entry. This call only affects the caller's recent players list (unidirectional).
/// Recent player updates will be batched and uploaded by XSAPI using the background queue provided
/// during XblInitialize. To force an immediate flush, call XblMultiplayerActivityFlushRecentPlayersAsync.
/// </summary>
...
/// <summary>
/// Immediately writes and pending recent players updates to XboxLive service. Note that calling this
/// API is optional; updates will automatically be uploaded periodically on a background task queue.
...
/// Titles can subscribe to changes in activity for other users via <TODO>.
/// </summary>
/// <param name="xblContext">Xbox live context for the local user.</param>
/// <param name="activityInfo">
/// Struct describing the activity. The "maxPlayers" and "currentPlayers" fields are optional and
/// will be ignored if they are set to 0. The value of the "platform" field will be ignored and XSAPI 
/// will automatically set the activity as being on the correct local platform.
/// </param>
...
/// </summary>
/// <param name="async">The AsyncBlock for this operation.</param>
/// <param name="bufferSize">The size of the result buffer. Use <see cref="XblMultiplayerActivityGetActivityResultSize"/>
/// to get the required buffer size.</param>
...
/// <summary>
/// Clear the multiplayer activity for the local user. Titles should delete the activity for a user as
/// soon as they leave the multiplayer activity. A user's activity will be automatically cleared by 
/// a presence check if the title fails to delete it manually.
/// </summary>
/// <param name="xblContext">Xbox live context for the local user.</param>
/// <param name="async">The AsyncBlock for this operation.</param>
/// <returns>HRESULT return code for this API operation.</returns>
...
/// If the title is configured for cross platform invites, setting this to true will send an invitation to all
/// platform endpoints the title supports. If set to false, the invite will only be sent to the platform of the sender.
/// If cross platform invites are not configured, the invite will always be sent to just the sender's platform.
/// </param>
/// <param>(Optional) Connection string that the peer will use to join the game.</param>
/// <param name="async">The AsyncBlock for this operation.</param>
/// <returns>HRESULT return code for this API operation.</returns>
/// <remarks>
/// The result of the asynchronous operation can be obtained by calling <see cref="XAsyncGetStatus"/> 
