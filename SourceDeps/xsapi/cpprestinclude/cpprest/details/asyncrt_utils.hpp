...
#if !defined(ANDROID) && !defined(__ANDROID__) && !defined(__ORBIS__)
...
#elif (defined(ANDROID) || defined(__ANDROID__) || defined(__ORBIS__))
scoped_c_thread_locale::scoped_c_thread_locale() {}
scoped_c_thread_locale::~scoped_c_thread_locale() {}
#else
scoped_c_thread_locale::scoped_c_thread_locale()
...
datetime __cdecl datetime::from_string(const utility::string_t& dateString, date_format format)
{
    ...
    if (format == RFC_1123)
    {
        ...
        iss >> std::get_time(&output, "%a, %d %b %Y %H:%M:%S GMT");
    }
    else
    {
        // Try to extract the fractional second from the timestamp
        ...
        std::istringstream iss(input);
        iss >> std::get_time(&output, "%Y-%m-%dT%H:%M:%SZ");

        if (iss.fail())
        {
            // Clear fail flag and rewind before parsing again.
            iss.clear();
            iss.seekg(0);
            iss >> std::get_time(&output, "%Y%m%dT%H:%M:%SZ");
            ...
            if (iss.fail())
            {
                ...
                iss.clear();
                iss.seekg(0);
                iss >> std::get_time(&output, "%H:%M:%SZ");

                if (iss.fail())
                {
                    iss.clear();
                    iss.seekg(0);
                    iss >> std::get_time(&output, "%Y-%m-%d");
    ...
    dateTime.year = output.tm_year + 1900;
    dateTime.month = output.tm_mon + 1;
    dateTime.day = output.tm_mday;
    dateTime.hour = output.tm_hour;
    dateTime.minute = output.tm_min;
    dateTime.second = output.tm_sec;
    dateTime.microsecond = 0;
    
    time_t time = 0;
    if (sceRtcGetTime_t(&dateTime, &time) < 0) {
        return datetime();
    }
#else
