...
{
    auto cstr = Utils::StringFromStringT(value);
    XblHttpCallRequestSetRequestBodyString(m_callHandle, cstr.c_str());
}
...
{
    auto cstr = Utils::SerializeJson(value);
    XblHttpCallRequestSetRequestBodyString(m_callHandle, cstr.c_str());
}
...
{
    auto headerName = Utils::StringFromStringT(Name);
    auto headerValue = Utils::StringFromStringT(Value);

    XblHttpCallRequestSetHeader(m_callHandle, headerName.c_str(), headerValue.c_str(), false);
}
