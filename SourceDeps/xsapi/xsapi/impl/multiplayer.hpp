...
tournament_reference::tournament_reference(const XblTournamentReference& tournamentReference)
    : m_tournamentReference(tournamentReference)
{
}

string_t tournament_reference::definition_name() const
{
    return Utils::StringTFromUtf8(m_tournamentReference.TournamentId);
}

string_t tournament_reference::tournament_id() const
{
    return Utils::StringTFromUtf8(m_tournamentReference.TournamentId);
}

string_t tournament_reference::organizer() const
{
    return Utils::StringTFromUtf8(m_tournamentReference.Organizer);
}

string_t tournament_reference::service_configuration_id() const
{
    return Utils::StringTFromUtf8(m_tournamentReference.Scid);
}

bool tournament_reference::is_null() const
{
    return m_tournamentReference.Scid[0] == 0 ||
        m_tournamentReference.DefinitionName[0] == 0 ||
        m_tournamentReference.Organizer[0] == 0 ||
        m_tournamentReference.TournamentId[0] == 0;
}

tournament_team_result::tournament_team_result(_In_opt_ const XblTournamentTeamResult* teamResult)
{
    m_result = teamResult ? teamResult->GameResult : XblTournamentGameResultWithRank{};
}

tournament_team_result::tournament_team_result(_In_ const XblTournamentGameResultWithRank& gameResult)
    : m_result(gameResult)
{
}
...
{
    m_result.Result = static_cast<XblTournamentGameResult>(state);
    m_result.Ranking = ranking;
}


tournament_game_result_state tournament_team_result::state() const
{
    return static_cast<tournament_game_result_state>(m_result.Result);
}

uint64_t tournament_team_result::ranking() const
{
    return m_result.Ranking;
}
...
{
    return m_capabilities.Team;
}

void multiplayer_session_capabilities::set_team(_In_ bool team)
{
    m_capabilities.Team = team;
}

bool multiplayer_session_capabilities::arbitration() const
{
    return m_capabilities.Arbitration;
}

void multiplayer_session_capabilities::set_arbitration(_In_ bool arbitration)
{
    m_capabilities.Arbitration = arbitration;
}
...
{
    m_customJson.Parse(Utils::StringFromStringT(customJson).c_str());
    m_measurementsJson.SetObject();
    rapidjson::Document::AllocatorType& allocator = m_measurementsJson.GetAllocator();
    m_measurementsJson.AddMember("latency", static_cast<int64_t>( m_latency.count() ), allocator);
    m_measurementsJson.AddMember("bandwidthDown", m_bandwidthDown, allocator);
    m_measurementsJson.AddMember("bandwidthUp", m_bandwidthUp, allocator);
    m_measurementsJson.AddMember("custom", m_customJson, allocator);

    m_customJsonWeb = web::json::value::parse(customJson);
}

multiplayer_quality_of_service_measurements::multiplayer_quality_of_service_measurements(
    _In_ const string_t& memberDeviceToken,
    _In_ const rapidjson::Value& measurementsJson
)
    : m_memberDeviceToken(memberDeviceToken)

{
    m_latency = std::chrono::milliseconds(Utils::ExtractJsonUint64(m_measurementsJson, "latency"));
    m_bandwidthDown = Utils::ExtractJsonUint64(m_measurementsJson, "bandwidthDown");
    m_bandwidthUp = Utils::ExtractJsonUint64(m_measurementsJson, "bandwidthUp");
    m_measurementsJson.CopyFrom(measurementsJson, m_measurementsJson.GetAllocator());
    m_customJson.CopyFrom(Utils::ExtractJsonField(m_measurementsJson, "custom", false), m_customJson.GetAllocator());
    m_customJsonWeb = Utils::WebJsonFromRapidJson(m_customJson);
}
...
std::chrono::milliseconds multiplayer_session_constants::arbitration_timeout() const
{
    return std::chrono::milliseconds(m_constants->ArbitrationTimeout);
}

std::chrono::milliseconds multiplayer_session_constants::forfeit_timeout() const
{
    return std::chrono::milliseconds(m_constants->ForfeitTimeout);
}
...
{
    return m_constants->SessionCapabilities.Team;
}
...
{
    return m_constants->SessionCapabilities.Arbitration;
}
...
{
    XblMultiplayerSessionDuplicateHandle(sessionHandle, &m_sessionHandle);
    m_arbitrationServer = XblMultiplayerSessionArbitrationServer(m_sessionHandle);
}

multiplayer_session_arbitration_server::multiplayer_session_arbitration_server(
    const multiplayer_session_arbitration_server& other
)
...
multiplayer_session_arbitration_server& multiplayer_session_arbitration_server::operator=(
    multiplayer_session_arbitration_server other
)
{
    std::swap(m_sessionHandle, other.m_sessionHandle);
    m_arbitrationServer = other.m_arbitrationServer;
...
utility::datetime multiplayer_session_arbitration_server::arbitration_start_time() const
{
    return m_arbitrationServer ? Utils::DatetimeFromTimeT(m_arbitrationServer->ArbitrationStartTime) : utility::datetime();
}

xbox::services::tournaments::tournament_arbitration_state multiplayer_session_arbitration_server::result_state() const
{
    return m_arbitrationServer ? static_cast<xbox::services::tournaments::tournament_arbitration_state>(m_arbitrationServer->ResultState)
        : xbox::services::tournaments::tournament_arbitration_state::none;
}

xbox::services::tournaments::tournament_game_result_source multiplayer_session_arbitration_server::result_source() const
{
    return m_arbitrationServer ? static_cast<xbox::services::tournaments::tournament_game_result_source>(m_arbitrationServer->ResultSource)
        : xbox::services::tournaments::tournament_game_result_source::none;
}

uint32_t multiplayer_session_arbitration_server::result_confidence_level() const
{
    return m_arbitrationServer ? m_arbitrationServer->ResultConfidenceLevel : 0;
}

std::unordered_map<string_t, xbox::services::tournaments::tournament_team_result> multiplayer_session_arbitration_server::results() const
{
    std::unordered_map<string_t, xbox::services::tournaments::tournament_team_result> out;

    if (m_arbitrationServer)
    {
        for (uint32_t i = 0; i < m_arbitrationServer->ResultsCount; ++i)
        {
            auto& result = m_arbitrationServer->Results[i];
            out.insert(std::make_pair(Utils::StringTFromUtf8(result.Team), &result));
        }
    }
    return out;
}
...
{
    XblMultiplayerSessionDuplicateHandle(sessionHandle, &m_sessionHandle);
    m_server = XblMultiplayerSessionTournamentsServer(m_sessionHandle);
}

multiplayer_session_tournaments_server::multiplayer_session_tournaments_server(
    const multiplayer_session_tournaments_server& other
)
    : m_server(other.m_server)
{
    XblMultiplayerSessionDuplicateHandle(other.m_sessionHandle, &m_sessionHandle);
...
multiplayer_session_tournaments_server& multiplayer_session_tournaments_server::operator=(
    multiplayer_session_tournaments_server other
)
{
    std::swap(m_sessionHandle, other.m_sessionHandle);
    m_server = other.m_server;
...
xbox::services::tournaments::tournament_reference multiplayer_session_tournaments_server::tournament_reference() const
{
    return m_server ? xbox::services::tournaments::tournament_reference(m_server->TournamentReference) : xbox::services::tournaments::tournament_reference(XblTournamentReference{});
}

std::unordered_map<string_t, multiplayer_session_reference> multiplayer_session_tournaments_server::teams() const
{
    std::unordered_map<string_t, multiplayer_session_reference> out;

    if (m_server)
    {
        for (uint32_t i = 0; i < m_server->TeamsCount; ++i)
        {
            auto& team = m_server->Teams[i];
            out[Utils::StringTFromUtf8(team.TeamId)] = multiplayer_session_reference(team.TeamSessionReference);
        }
    }
    return out;
}

xbox::services::tournaments::tournament_registration_state multiplayer_session_tournaments_server::registration_state() const
{
    return m_server ? static_cast<xbox::services::tournaments::tournament_registration_state>(m_server->RegistrationState) : xbox::services::tournaments::tournament_registration_state::unknown;
}

xbox::services::tournaments::tournament_registration_reason multiplayer_session_tournaments_server::registration_reason() const
{
    return m_server ? static_cast<xbox::services::tournaments::tournament_registration_reason>(m_server->RegistrationReason) : xbox::services::tournaments::tournament_registration_reason::unknown;
}

utility::datetime multiplayer_session_tournaments_server::next_game_start_time() const
{
    return m_server ? Utils::DatetimeFromTimeT(m_server->NextGameStartTime) : utility::datetime();
}

multiplayer_session_reference multiplayer_session_tournaments_server::next_game_session_reference() const
{
    return m_server ? multiplayer_session_reference(m_server->NextGameSessionReference) : multiplayer_session_reference();
}

utility::datetime multiplayer_session_tournaments_server::last_game_end_time() const
{
    return m_server ? Utils::DatetimeFromTimeT(m_server->LastGameEndTime) : utility::datetime();
}

xbox::services::tournaments::tournament_team_result multiplayer_session_tournaments_server::last_team_result() const
{
    return m_server ? xbox::services::tournaments::tournament_team_result(m_server->LastTeamResult.GameResult) : xbox::services::tournaments::tournament_team_result(nullptr);
}

xbox::services::tournaments::tournament_game_result_source multiplayer_session_tournaments_server::last_game_result_source() const
{
    return m_server ? static_cast<xbox::services::tournaments::tournament_game_result_source>(m_server->LastGameResultSource) : xbox::services::tournaments::tournament_game_result_source::none;
}
...
string_t multiplayer_session_member::team_id() const
{
    return Utils::StringTFromUtf8(m_member->TeamId);
}
...
xbox::services::tournaments::tournament_arbitration_status multiplayer_session_member::arbitration_status() const
{
    return static_cast<xbox::services::tournaments::tournament_arbitration_status>(m_member->ArbitrationStatus);
}
...
std::unordered_map<string_t, xbox::services::tournaments::tournament_team_result> multiplayer_session_member::results() const
{
    return std::unordered_map<string_t, xbox::services::tournaments::tournament_team_result>();
}
...
std::shared_ptr<std::vector<multiplayer_quality_of_service_measurements>> multiplayer_session_member::member_measurements() const
{
    ...
    {
        rapidjson::Document measurementsJson;
        measurementsJson.Parse(m_member->QosMeasurementsJson);
        for (const auto& pair : measurementsJson.GetObject())
        {
            out->push_back(multiplayer_quality_of_service_measurements(Utils::StringTFromUtf8(pair.name.GetString()), pair.value));
        }
    }
    return out;
}
...
multiplayer_session_reference multiplayer_session_member::tournament_team_session_reference() const
{
    return multiplayer_session_reference(m_member->TournamentTeamSessionReference);
}
...
xbox::services::tournaments::tournament_arbitration_status multiplayer_session::arbitration_status() const
{
    return static_cast<xbox::services::tournaments::tournament_arbitration_status>(XblMultiplayerSessionArbitrationStatus(m_handle));
}
...
multiplayer_session_tournaments_server multiplayer_session::tournaments_server() const
{
    return multiplayer_session_tournaments_server(m_handle);
}

multiplayer_session_arbitration_server multiplayer_session::arbitration_server() const
{
    return multiplayer_session_arbitration_server(m_handle);
}
...
std::error_code multiplayer_session::set_arbitration_timeouts(
    _In_ std::chrono::milliseconds arbitrationTimeout,
    _In_ std::chrono::milliseconds forfeitTimeout
)
{
    auto hr = XblMultiplayerSessionConstantsSetArbitrationTimeouts(
        m_handle,
        arbitrationTimeout.count(),
        forfeitTimeout.count()
    );
    return Utils::ConvertHr(hr);
}
...
)
{
    ...
    {
        rapidjson::Value measurementJson;
        measurementJson.CopyFrom(measurement.m_measurementsJson, json.GetAllocator());
        json.AddMember(rapidjson::Value(Utils::StringFromStringT(measurement.m_memberDeviceToken).c_str(), json.GetAllocator()).Move(), measurementJson, json.GetAllocator());
    }
    auto hr = XblMultiplayerSessionCurrentUserSetQosMeasurements(m_handle, Utils::SerializeJson(json).data());
    return Utils::ConvertHr(hr);
}
...
std::error_code multiplayer_session::set_current_user_member_arbitration_results(
...
)
{
    ...
    return std::make_error_code(xbox::services::xbox_live_error_code::unsupported);
}
...
{
    // TODO
    UNREFERENCED_PARAMETER(targetSessionReference);
    UNREFERENCED_PARAMETER(originSessionReference);
    return pplx::task_from_result(xbox_live_result<string_t>(Utils::ConvertHr(E_NOTIMPL)));
}
