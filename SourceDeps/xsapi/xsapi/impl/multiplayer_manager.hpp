...
string_t multiplayer_member::team_id() const
{
    return Utils::StringTFromUtf8(m_internalMember.TeamId);
}
...
xbox::services::tournaments::tournament_team_result multiplayer_lobby_session::last_tournament_team_result() const
{
    return xbox::services::tournaments::tournament_team_result(XblMultiplayerManagerLobbySessionLastTournamentTeamResult());
}
...
std::unordered_map<string_t, multiplayer_session_reference> multiplayer_game_session::tournament_teams() const
{
     return std::unordered_map<string_t, multiplayer_session_reference>();
}

std::unordered_map<string_t, xbox::services::tournaments::tournament_team_result> multiplayer_game_session::tournament_team_results() const
{
...
xbox::services::tournaments::tournament_registration_state tournament_registration_state_changed_event_args::registration_state() const
{
    XblTournamentRegistrationState state;
    XblMultiplayerEventArgsTournamentRegistrationStateChanged(m_argsHandle, &state, nullptr);
    return static_cast<xbox::services::tournaments::tournament_registration_state>(state);
}

xbox::services::tournaments::tournament_registration_reason tournament_registration_state_changed_event_args::registration_reason() const
{
    XblTournamentRegistrationReason reason;
    XblMultiplayerEventArgsTournamentRegistrationStateChanged(m_argsHandle, nullptr, &reason);
    return static_cast<xbox::services::tournaments::tournament_registration_reason>(reason);
}

utility::datetime tournament_game_session_ready_event_args::start_time() const
{
    time_t startTime;
    XblMultiplayerEventArgsTournamentGameSessionReady(m_argsHandle, &startTime);
    return Utils::DatetimeFromTimeT(startTime);
}
...
std::shared_ptr<multiplayer_event_args> multiplayer_event::event_args()
{
    if (m_eventArgs == nullptr)
    {
        ...
        {
            m_eventArgs = std::make_shared<tournament_registration_state_changed_event_args>(m_internalEvent->EventArgsHandle);
            break;
        }
        case XblMultiplayerEventType::TournamentGameSessionReady:
        {
            m_eventArgs = std::make_shared<tournament_game_session_ready_event_args>(m_internalEvent->EventArgsHandle);
            break;
        }
        default: break;
        }
...
{
    rapidjson::Document measurementsJson(rapidjson::kObjectType);
    rapidjson::Document::AllocatorType& allocator = measurementsJson.GetAllocator();
    for (const auto& measurement : *measurements)
    {
        rapidjson::Value jsonMeasurement(rapidjson::kObjectType);
        jsonMeasurement.AddMember("latency", static_cast<int64_t>( measurement.latency().count() ), allocator);
        jsonMeasurement.AddMember("bandwidthDown", measurement.bandwidth_down_in_kilobits_per_second(), allocator);
        jsonMeasurement.AddMember("bandwidthUp", measurement.bandwidth_up_in_kilobits_per_second(), allocator);
        rapidjson::Value measurementCustomJson;
        measurementCustomJson.CopyFrom(Utils::RapidJsonFromWebJson(measurement.custom_json()), allocator);
        jsonMeasurement.AddMember("custom", measurementCustomJson, allocator);

        measurementsJson.AddMember(rapidjson::Value (Utils::StringFromStringT(measurement.member_device_token()).c_str(), allocator).Move(), jsonMeasurement, allocator);
    }
    auto measurementsJsonString = Utils::SerializeJson(measurementsJson);
