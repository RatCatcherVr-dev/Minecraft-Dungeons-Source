...
typedef enum XalShowUrlType
{
    ...
    /// whichever browser would give the best user experience.</summary>
...
//-----------------------------------------------------------------------------
// Context mapping (ignored on all Sua and OneCore platforms)

/// <summary>
/// Map user handle to callback context event handler.
/// </summary>
/// <param name="context">Optional pointer to data used by the event handler.
/// </param>
/// <param name="user">The user handle to provide a user context for.</param>
/// <returns>The user context pointer.</returns>
/// <remarks>
/// This event is raised just before any of the other events, when they are
/// raised outside an add user operation.
///
/// The context pointer returned by this function will be passed in to the
/// subsequent event handler call as the userContext argument.
/// During add user operations, the user context used for events is the one
/// passed to the add user operation itself.
/// </remarks>
typedef void* (XalPlatformUserHandleToContextEventHandler)(
