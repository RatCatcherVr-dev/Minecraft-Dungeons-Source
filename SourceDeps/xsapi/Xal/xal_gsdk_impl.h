...
HRESULT XalTryAddDefaultUserSilentlyAsync(
    _In_opt_ void* /*userContext*/, // user context is not used on GameCore
    _In_ XAsyncBlock* async
) noexcept
{
...
{
    XUserLocalId gdkLocalId = { localId.value };
    return XUserFindUserByLocalId(gdkLocalId, user);
}
...
{
    static_assert(sizeof(XalUserLocalId) == sizeof(XUserLocalId), "Xal / GDK mismatch");
    static_assert(sizeof(XalUserLocalId::value) == sizeof(XUserLocalId::value), "Xal / GDK mismatch");

    HRESULT hr = XUserGetLocalId(user, reinterpret_cast<XUserLocalId*>(localId));
    return hr;
}
