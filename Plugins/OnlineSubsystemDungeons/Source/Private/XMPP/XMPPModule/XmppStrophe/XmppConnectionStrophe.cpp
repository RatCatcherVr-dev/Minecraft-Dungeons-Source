...
#include "XmppStrophe/XmppRelayTokenStrophe.h"
	...
	: LoginStatus(EXmppLoginStatus::NotStarted)
{
	...
	RelayTokenStrophe = MakeShared<FXmppRelayTokenStrophe, ESPMode::ThreadSafe>(*this);
}
...
void FXmppConnectionStrophe::Login(const FString& UserId, const FString& Auth)
{
	...
	else
	{
		...
		else
		{
			...
			{
				FString ServerUrl = (ServerConfiguration.ServerPort == 0) ? FString::Printf(TEXT("%s"), *ServerConfiguration.ServerAddr) : FString::Printf(TEXT("%s:%i"), *ServerConfiguration.ServerAddr, ServerConfiguration.ServerPort);
				WebsocketConnection = MakeUnique<FStropheWebsocketConnection>(StropheContext, ServerUrl);
...
void FXmppConnectionStrophe::Logout()
{
	...
	RelayTokenStrophe->OnDisconnect();
}
...
FXmppRelayTokenPtr FXmppConnectionStrophe::RelayToken()
{
	return RelayTokenStrophe;
}
...
void FXmppConnectionStrophe::ReceiveStanza(const FStropheStanza& Stanza)
{
	...
	if (RelayTokenStrophe.IsValid())
	{
		if (RelayTokenStrophe->ReceiveStanza(Stanza))
		{
			UE_LOG(LogXmpp, VeryVerbose, TEXT("%s Stanza handled by RelayTokenStrophe"), *Stanza.GetName());
			return;
		}
	}
