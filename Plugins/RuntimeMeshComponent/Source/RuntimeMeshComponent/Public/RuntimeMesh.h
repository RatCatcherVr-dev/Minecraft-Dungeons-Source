...
public:
	...
	bool IsAsyncCollisionCookingComplete() const;

	//D11.SC Dump out our mesh data
...
public:
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection(SectionIndex, LOD, bWantsHighPrecisionTangents, bWantsHighPrecisionUVs, NumUVs, bWants32BitIndices, bCreateCollision, UpdateFrequency);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection<VertexType0, IndexType>(SectionIndex, LOD, InVertices0, InTriangles, bCreateCollision, UpdateFrequency, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection<VertexType0, IndexType>(SectionIndex, LOD, InVertices0, InTriangles, BoundingBox, bCreateCollision, UpdateFrequency, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSectionDualBuffer<VertexType0, VertexType1, IndexType>(SectionIndex, LOD, InVertices0,
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSectionTripleBuffer<VertexType0, VertexType1, VertexType2, IndexType>(SectionIndex, LOD,
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection<VertexType0>(SectionId, LOD, InVertices0, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection<VertexType0>(SectionId, LOD, InVertices0, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection<VertexType0, IndexType>(SectionId, LOD, InVertices0, InTriangles, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection<VertexType0, IndexType>(SectionId, LOD, InVertices0, InTriangles, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionDualBuffer<VertexType0, VertexType1>(SectionId, LOD, InVertices0, InVertices1, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionDualBuffer<VertexType0, VertexType1>(SectionId, LOD, InVertices0, InVertices1, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionDualBuffer<VertexType0, VertexType1, IndexType>(SectionId, LOD, InVertices0, InVertices1, InTriangles, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionDualBuffer<VertexType0, VertexType1, IndexType>(SectionId, LOD, InVertices0, InVertices1, InTriangles, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionTripleBuffer<VertexType0, VertexType1, VertexType2>(SectionId, LOD, InVertices0, InVertices1, InVertices2, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionTripleBuffer<VertexType0, VertexType1, VertexType2>(SectionId, LOD, InVertices0, InVertices1, InVertices2, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionTripleBuffer<VertexType0, VertexType1, VertexType2, IndexType>(SectionId, LOD, InVertices0, InVertices1, InVertices2, InTriangles, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionTripleBuffer<VertexType0, VertexType1, VertexType2, IndexType>(SectionId, LOD, InVertices0, InVertices1, InVertices2, InTriangles, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionPrimaryBuffer<VertexType0>(SectionId, LOD, InVertices0, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionPrimaryBuffer<VertexType0>(SectionId, LOD, InVertices0, BoundingBox, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionSecondaryBuffer<VertexType1>(SectionId, LOD, InVertices1, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionTertiaryBuffer<VertexType2>(SectionId, LOD, InVertices2, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionTriangles<IndexType>(SectionId, LOD, InTriangles, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection(SectionId, LOD, MeshData, bCreateCollision, UpdateFrequency, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSectionByMove(SectionId, LOD, MeshData, bCreateCollision, UpdateFrequency, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection(SectionId, LOD, MeshData->GetMeshBuilder(), bCreateCollision, UpdateFrequency/*, UpdateFlags*/);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection(SectionId, LOD, MeshData, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionByMove(SectionId, LOD, MeshData, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection(SectionId, LOD, MeshData->GetMeshBuilder()/*, UpdateFlags*/);
	}
	...
	{
		check(IsInGameThread());
		return GetRuntimeMeshData()->BeginSectionUpdate(SectionId, LOD, UpdateFlags);
	}

	TUniquePtr<FRuntimeMeshScopedUpdater> GetSectionReadonly(int32 SectionId, int32 LOD)
	{
		check(IsInGameThread());
		return GetRuntimeMeshData()->GetSectionReadonly(SectionId, LOD);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, Colors, Tangents,
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, UV1, Colors, Tangents,
			bCreateCollision, UpdateFrequency, UpdateFlags, bUseHighPrecisionTangents, bUseHighPrecisionUVs);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection(SectionIndex, LOD, Vertices, Normals, UV0, Colors, Tangents, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection(SectionIndex, LOD, Vertices, Normals, UV0, UV1, Colors, Tangents, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, Colors, Tangents, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, UV1, Colors, Tangents, UpdateFlags);
	}
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSection_Blueprint(SectionIndex, LOD, Vertices, Triangles, Normals, Tangents, UV0, UV1, Colors,
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSection_Blueprint(SectionIndex, LOD, Vertices, Triangles, Normals, Tangents, UV0, UV1, Colors,
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->CreateMeshSectionPacked_Blueprint(SectionIndex, LOD, Vertices, Triangles, bCreateCollision, bCalculateNormalTangent, bShouldCreateHardTangents,
	...
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->UpdateMeshSectionPacked_Blueprint(SectionIndex, LOD, Vertices, Triangles, bCalculateNormalTangent, bShouldCreateHardTangents, bGenerateTessellationTriangles);
	}
	...
	UFUNCTION(BlueprintCallable, Category = "Components|RuntimeMesh")
	void SetSectionTessellationTriangles(int32 SectionId, int32 LOD, const TArray<int32>& Triangles)
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->SetSectionTessellationTriangles(SectionId, LOD, Triangles);
	}

	template<typename IndexType>
	void SetSectionTessellationTriangles(int32 SectionId, int32 LOD,  const TArray<IndexType>& Triangles)
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->SetSectionTessellationTriangles(SectionId, LOD, Triangles);
	}
	...
	void TransferMeshCollisionSection(int32 CollisionSectionIndex, TArray<FVector>& Vertices, TArray<int32>& Triangles) //transfers ownership of memory arrays
	{
		check(IsInGameThread());
		GetRuntimeMeshData()->TransferMeshCollisionSection(CollisionSectionIndex, Vertices, Triangles);
	}
