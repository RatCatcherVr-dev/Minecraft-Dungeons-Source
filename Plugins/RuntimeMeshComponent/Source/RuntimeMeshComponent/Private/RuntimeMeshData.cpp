...
DECLARE_CYCLE_STAT(TEXT("RM - Get Section Ids"), STAT_RuntimeMesh_GetSectionIndexs, STATGROUP_RuntimeMesh);
...
void FRuntimeMeshData::CheckUpdate(bool bUseHighPrecisionTangents, bool bUseHighPrecisionUVs, int32 NumUVs, bool b32BitIndices, int32 SectionIndex, int32 LOD, bool bShouldCheckIndexType,
	bool bCheckTangentVertexStream, bool bCheckUVVertexStream) const
{
	...
	if (bCheckTangentVertexStream && !Section->CheckTangentBuffer(bUseHighPrecisionTangents, LOD))
	{
		UE_LOG(RuntimeMeshLog, Fatal, TEXT("Supplied vertex type does not match stream 1 for mesh section %d."), SectionIndex);
	}

	if (bCheckUVVertexStream && !Section->CheckUVBuffer(bUseHighPrecisionUVs, NumUVs, LOD))
	{
		UE_LOG(RuntimeMeshLog, Fatal, TEXT("Supplied vertex type does not match stream 2 for mesh section %d."), SectionIndex);
	}

	if (bShouldCheckIndexType && !Section->CheckIndexBufferSize(b32BitIndices, LOD))
	{
...
void FRuntimeMeshData::CreateMeshSection(int32 SectionIndex, int32 LOD, bool bWantsHighPrecisionTangents, bool bWantsHighPrecisionUVs, int32 NumUVs, bool bWants32BitIndices, bool bCreateCollision, EUpdateFrequency UpdateFrequency /*= EUpdateFrequency::Average*/)
{
	...
	auto NewSection = CreateOrResetSection(SectionIndex, LOD, bWantsHighPrecisionTangents, bWantsHighPrecisionUVs, NumUVs, bWants32BitIndices, UpdateFrequency);
	
	// Track collision status and update collision information if necessary
	NewSection->SetCollisionEnabled(bCreateCollision);

	// Finalize section.
	CreateSectionInternal(SectionIndex, LOD, ESectionUpdateFlags::None);
}

void FRuntimeMeshData::CreateMeshSection(int32 SectionIndex, int32 LOD, const TSharedPtr<FRuntimeMeshBuilder>& MeshData, bool bCreateCollision /*= false*/, EUpdateFrequency UpdateFrequency /*= EUpdateFrequency::Average*/, ESectionUpdateFlags UpdateFlags /*= ESectionUpdateFlags::None*/)
{
	...
	auto NewSection = CreateOrResetSection(SectionIndex, LOD, MeshData->IsUsingHighPrecisionTangents(), MeshData->IsUsingHighPrecisionUVs(), MeshData->NumUVChannels(), MeshData->IsUsing32BitIndices(), UpdateFrequency);
	...
	// Finalize section.
	CreateSectionInternal(SectionIndex, LOD, UpdateFlags);
}

void FRuntimeMeshData::CreateMeshSectionByMove(int32 SectionIndex, int32 LOD, const TSharedPtr<FRuntimeMeshBuilder>& MeshData, bool bCreateCollision /*= false*/, EUpdateFrequency UpdateFrequency /*= EUpdateFrequency::Average*/, ESectionUpdateFlags UpdateFlags /*= ESectionUpdateFlags::None*/)
{
	...
	NewSection->UpdatePositionBuffer(MeshData->GetPositionStream(), LOD, true);
	NewSection->UpdateTangentsBuffer(MeshData->GetTangentStream(), LOD, true);
	NewSection->UpdateUVsBuffer(MeshData->GetUVStream(), LOD, true);
	NewSection->UpdateColorBuffer(MeshData->GetColorStream(), LOD, true);
	NewSection->UpdateIndexBuffer(MeshData->GetIndexStream(), LOD, true);
...
void FRuntimeMeshData::UpdateMeshSection(int32 SectionIndex, int32 LOD, const TSharedPtr<FRuntimeMeshBuilder>& MeshData, ESectionUpdateFlags UpdateFlags /*= ESectionUpdateFlags::None*/)
{
	SCOPE_CYCLE_COUNTER(STAT_RuntimeMesh_UpdateMeshSection_MeshData);

	FRuntimeMeshScopeLock Lock(SyncRoot);

	CheckUpdate(MeshData->IsUsingHighPrecisionTangents(), MeshData->IsUsingHighPrecisionUVs(), MeshData->NumUVChannels(), MeshData->IsUsing32BitIndices(), SectionIndex, LOD, true, true, true);
	...
	UpdateSectionInternal(SectionIndex, LOD, BuffersToUpdate, UpdateFlags);
}

void FRuntimeMeshData::UpdateMeshSectionByMove(int32 SectionIndex, int32 LOD, const TSharedPtr<FRuntimeMeshBuilder>& MeshData, ESectionUpdateFlags UpdateFlags /*= ESectionUpdateFlags::None*/)
{
	...
	Section->UpdatePositionBuffer(MeshData->GetPositionStream(), LOD, true);
	Section->UpdateTangentsBuffer(MeshData->GetTangentStream(), LOD, true);
	Section->UpdateUVsBuffer(MeshData->GetUVStream(), LOD, true);
	Section->UpdateColorBuffer(MeshData->GetColorStream(), LOD, true);
	Section->UpdateIndexBuffer(MeshData->GetIndexStream(), LOD, true);
...
TUniquePtr<FRuntimeMeshScopedUpdater> FRuntimeMeshData::BeginSectionUpdate(int32 SectionIndex, int32 LOD, ESectionUpdateFlags UpdateFlags /*= ESectionUpdateFlags::None*/)
{
	...
	return Section->GetSectionMeshUpdater(this->AsShared(), SectionIndex, LOD, UpdateFlags, SyncRoot.Get(), false);
}

TUniquePtr<FRuntimeMeshScopedUpdater> FRuntimeMeshData::GetSectionReadonly(int32 SectionIndex, int32 LOD)
{
	check(DoesSectionExist(SectionIndex));
	...
	return Section->GetSectionMeshUpdater(this->AsShared(), SectionIndex, LOD, ESectionUpdateFlags::None, SyncRoot.Get(), true);
}
...
void FRuntimeMeshData::EndSectionUpdate(FRuntimeMeshScopedUpdater* Updater, ERuntimeMeshBuffersToUpdate BuffersToUpdate, const FBox* BoundingBox /*= nullptr*/)
{
	...
	UpdateSectionInternal(Updater->SectionIndex, Updater->LOD, BuffersToUpdate, Updater->UpdateFlags);
}

void FRuntimeMeshData::CreateMeshSectionFromComponents(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals,
	...
	bool bUseHighPrecisionTangents, bool bUseHighPrecisionUVs, bool bWantsSecondUV)
{
	...
	// Create the section
	auto NewSection = CreateOrResetSectionForBlueprint(SectionIndex, LOD, bWantsSecondUV, bUseHighPrecisionTangents, bUseHighPrecisionUVs, UpdateFrequency);

	TSharedPtr<FRuntimeMeshAccessor> MeshData = NewSection->GetSectionMeshAccessor(LOD);
	...
	NewSection->UpdateIndexBuffer(Triangles, LOD);
...
void FRuntimeMeshData::UpdateMeshSectionFromComponents(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals,
...
//	CheckUpdate(GetStreamStructure<FVector>(), GetStreamStructure<FRuntimeMeshNullVertex>(), GetStreamStructure<FColor>(), true, SectionIndex, true, true, false, true);
	...
	{
		TSharedPtr<FRuntimeMeshAccessor> MeshData = Section->GetSectionMeshAccessor(LOD);
	...
	if (Triangles.Num() > 0)
	{
		Section->UpdateIndexBuffer(Triangles,LOD);
...
void FRuntimeMeshData::UpdateMeshSectionColors(int32 SectionIndex, TArray<FColor>& Colors, ESectionUpdateFlags UpdateFlags /*= ESectionUpdateFlags::None*/)
{
	...
	TSharedPtr<FRuntimeMeshAccessor> MeshData = Section->GetSectionMeshAccessor(0);

	for (int i = 0; i < Colors.Num(); i++)
		MeshData->SetColor(i, Colors[i]);

	if (RenderProxy.IsValid())
		RenderProxy->UpdateSection_GameThread(SectionIndex, Section->GetSectionUpdateData(BuffersToUpdate, 0));
...
void FRuntimeMeshData::CreateMeshSection(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals,
...
{
	CreateMeshSectionFromComponents(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, TArray<FVector2D>(), [&Colors](int32 Index) -> FColor { return Colors[Index]; },
...
{
	CreateMeshSectionFromComponents(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, UV1, [&Colors](int32 Index) -> FColor { return Colors[Index]; },
		Colors.Num(), Tangents, bCreateCollision, UpdateFrequency, UpdateFlags, bUseHighPrecisionTangents, bUseHighPrecisionUVs, true);
}

void FRuntimeMeshData::UpdateMeshSection(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0,
	const TArray<FColor>& Colors, const TArray<FRuntimeMeshTangent>& Tangents, ESectionUpdateFlags UpdateFlags)
{
	UpdateMeshSectionFromComponents(SectionIndex, LOD, Vertices, TArray<int32>(), Normals, UV0, TArray<FVector2D>(),
...
{
	UpdateMeshSectionFromComponents(SectionIndex, LOD, Vertices, TArray<int32>(), Normals, UV0, UV1,
		[&Colors](int32 Index) -> FColor { return Colors[Index]; }, Colors.Num(), Tangents, UpdateFlags);
}

void FRuntimeMeshData::UpdateMeshSection(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals,
...
{
	UpdateMeshSectionFromComponents(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, TArray<FVector2D>(),
		[&Colors](int32 Index) -> FColor { return Colors[Index]; }, Colors.Num(), Tangents, UpdateFlags);
}
...
{
	UpdateMeshSectionFromComponents(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, UV1,
		[&Colors](int32 Index) -> FColor { return Colors[Index]; }, Colors.Num(), Tangents, UpdateFlags);
}

void FRuntimeMeshData::CreateMeshSection_Blueprint(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals,
	...
	bool bCalculateNormalTangent, bool bShouldCreateHardTangents, bool bGenerateTessellationTriangles, EUpdateFrequency UpdateFrequency, bool bUseHighPrecisionTangents, bool bUseHighPrecisionUVs)
{
	...
	CreateMeshSectionFromComponents(SectionIndex, LOD, Vertices, Triangles, Normals, UV0, UV1, [&VertexColors](int32 Index) -> FColor { return VertexColors[Index].ToFColor(false); },
...
void FRuntimeMeshData::UpdateMeshSection_Blueprint(int32 SectionIndex, int32 LOD, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FRuntimeMeshTangent>& Tangents,
...
void FRuntimeMeshData::CreateMeshSectionPacked_Blueprint(int32 SectionIndex, int32 LOD, const TArray<FRuntimeMeshBlueprintVertexSimple>& Vertices, const TArray<int32>& Triangles,
	bool bCreateCollision, bool bCalculateNormalTangent, bool bShouldCreateHardTangents, bool bGenerateTessellationTriangles, EUpdateFrequency UpdateFrequency, bool bUseHighPrecisionTangents, bool bUseHighPrecisionUVs)
{
	...
	// Create the section
	auto NewSection = CreateOrResetSectionForBlueprint(SectionIndex, LOD, false, bUseHighPrecisionTangents, bUseHighPrecisionUVs, UpdateFrequency);
...
void FRuntimeMeshData::UpdateMeshSectionPacked_Blueprint(int32 SectionIndex, int32 LOD, const TArray<FRuntimeMeshBlueprintVertexSimple>& Vertices, const TArray<int32>& Triangles,
...
TSharedPtr<const FRuntimeMeshAccessor> FRuntimeMeshData::GetReadonlyMeshAccessor(int32 SectionIndex, int32 LOD)
{
	...
	return ConstCastSharedPtr<const FRuntimeMeshAccessor, FRuntimeMeshAccessor>(Section->GetSectionMeshAccessor(LOD));
}
...
void FRuntimeMeshData::ClearMeshSection(int32 SectionIndex)
{
	...
	if (DoesSectionExist(SectionIndex))
	{
		...
		if (RenderProxy.IsValid())
		{
			RenderProxy->DeleteSection_GameThread(SectionIndex);
		}
...
void FRuntimeMeshData::ClearAllMeshSectionBuffers()
{
	FRuntimeMeshScopeLock Lock(SyncRoot);

	for (auto pSection : MeshSections)
	{
		if(pSection.IsValid())
			pSection->ClearAllMeshBuffers();
	}
}
...
{
	SCOPE_CYCLE_COUNTER(STAT_RuntimeMesh_GetSectionIndexs);
	...
	{
		if (MeshSections[SectionIndex].IsValid())
		{
...
void FRuntimeMeshData::TransferMeshCollisionSection(int32 CollisionSectionIndex, TArray<FVector>& Vertices, TArray<int32>& Triangles)
{
	//SCOPE_CYCLE_COUNTER(STAT_RuntimeMesh_TransferMeshCollisionSection);

	FRuntimeMeshScopeLock Lock(SyncRoot);

	auto& Section = MeshCollisionSections.FindOrAdd(CollisionSectionIndex);

	Section.VertexBuffer = std::move(Vertices);
	Section.IndexBuffer = std::move(Triangles);
...
FRuntimeMeshSectionPtr FRuntimeMeshData::CreateOrResetSection(int32 SectionIndex, int32 LOD, bool bInUseHighPrecisionTangents, bool bInUseHighPrecisionUVs,
	int32 InNumUVs, bool b32BitIndices, EUpdateFrequency UpdateFrequency)
{
	// Store section at index
	if (MeshSections.Num() <= SectionIndex)
	{
		MeshSections.SetNum(SectionIndex + 1);
	}

	// Create new section
	FRuntimeMeshSectionPtr NewSection = MeshSections[SectionIndex];

	if (!NewSection.IsValid())
	{
		NewSection = MakeShared<FRuntimeMeshSection, ESPMode::ThreadSafe>(bInUseHighPrecisionTangents, bInUseHighPrecisionUVs, InNumUVs, b32BitIndices, UpdateFrequency/*, LockFactory()*/);
		MeshSections[SectionIndex] = NewSection;
	}

	NewSection->CreateLODMeshData(LOD, bInUseHighPrecisionTangents, bInUseHighPrecisionUVs, InNumUVs, b32BitIndices);
	

	return NewSection;
}

FRuntimeMeshSectionPtr FRuntimeMeshData::CreateOrResetSectionForBlueprint(int32 SectionIndex, int32 LOD, bool bWantsSecondUV, bool bHighPrecisionTangents, bool bHighPrecisionUVs, EUpdateFrequency UpdateFrequency)
{
	return CreateOrResetSection(SectionIndex, LOD, bHighPrecisionTangents, bHighPrecisionUVs, bWantsSecondUV? 2 : 1, true, UpdateFrequency);
}




void FRuntimeMeshData::CreateSectionInternal(int32 SectionIndex, int32 LOD, ESectionUpdateFlags UpdateFlags)
{
	...
	if (RenderProxy.IsValid())
	{
		RenderProxy->CreateSection_GameThread(SectionIndex, Section->GetSectionCreationParams(LOD));
	}
	...
	HandleCommonSectionUpdateFlags(SectionIndex, LOD, UpdateFlags, BuffersToUpdate);
...
void FRuntimeMeshData::UpdateSectionInternal(int32 SectionIndex, int32 LOD, ERuntimeMeshBuffersToUpdate BuffersToUpdate, ESectionUpdateFlags UpdateFlags)
{
	...
	if (RenderProxy.IsValid())
	{
		RenderProxy->UpdateSection_GameThread(SectionIndex, Section->GetSectionUpdateData(BuffersToUpdate, LOD));
	}
...
void FRuntimeMeshData::HandleCommonSectionUpdateFlags(int32 SectionIndex, int32 LOD, ESectionUpdateFlags UpdateFlags, ERuntimeMeshBuffersToUpdate& BuffersToUpdate)
{
	...
	if (!!(UpdateFlags & ESectionUpdateFlags::CalculateNormalTangent) || !!(UpdateFlags & ESectionUpdateFlags::CalculateNormalTangentHard))
	{
		...
		URuntimeMeshLibrary::CalculateTangentsForMesh(Section->GetSectionMeshAccessor(LOD), !(UpdateFlags & ESectionUpdateFlags::CalculateNormalTangentHard));
		BuffersToUpdate |= ERuntimeMeshBuffersToUpdate::AllVertexBuffers;
	}
	...
	if (!!(UpdateFlags & ESectionUpdateFlags::CalculateTessellationIndices))
	{
		...
		URuntimeMeshLibrary::GenerateTessellationIndexBuffer(Section->GetSectionMeshAccessor(LOD), Section->GetTessellationIndexAccessor(LOD));
...
void FRuntimeMeshData::UpdateLocalBounds()
{
	...
	if (LocalBox.IsValid && FMath::IsNearlyZero(LocalBox.GetVolume()))
	{
		//Make sure that our mesh has some semblance of bounds, to prevent the occlusion culling triggering when we end up with flat planes
		const FVector BoxExtent = LocalBox.GetExtent();

		if (FMath::IsNearlyZero(BoxExtent.X))
		{
			LocalBox.Min.X -= 1.0f;
			LocalBox.Max.X += 1.0f;
		}

		if (FMath::IsNearlyZero(BoxExtent.Y))
		{
			LocalBox.Min.Y -= 1.0f;
			LocalBox.Max.Y += 1.0f;
		}

		if (FMath::IsNearlyZero(BoxExtent.Z))
		{
			LocalBox.Min.Z -= 1.0f;
			LocalBox.Max.Z += 1.0f;
		}
...
void FRuntimeMeshData::Initialize()
{
	...
	for (int32 SectionIndex = 0; SectionIndex < MeshSections.Num(); SectionIndex++)
	{
		if (MeshSections[SectionIndex].IsValid())
		{
			auto& MeshSection = MeshSections[SectionIndex];

			RenderProxy->CreateSection_GameThread(SectionIndex, MeshSection->GetSectionCreationParams(0)); 

			//DO sub LODs
			auto SubLods = MeshSection->GetSubLODS();

			for (const auto& SubLod : SubLods)
			{
				RenderProxy->CreateSection_GameThread(SectionIndex, MeshSection->GetSectionCreationParams(SubLod.Key));
			}
		}
	}
}
...
bool FRuntimeMeshData::GetPhysicsTriMeshData(struct FTriMeshCollisionData* CollisionData, bool InUseAllTriData)
{
	...
	for (int32 SectionIndex = 0; SectionIndex < MeshSections.Num(); SectionIndex++)
	{
		if (MeshSections[SectionIndex].IsValid() && MeshSections[SectionIndex]->IsCollisionEnabled())
		{
			TArray<FVector2D> UVs;
			int32 NumTriangles = MeshSections[SectionIndex]->GetCollisionData(CollisionData->Vertices, CollisionData->Indices, UVs);
...
int32 FRuntimeMeshData::GetSectionAndFaceFromCollisionFaceIndex(int32& FaceIndex) const
{
	...
	//4.22 merge change for 4.22 update, Steven should verify the slight altered logic here!
	for (int32 SectionIdx = 0; SectionIdx < MeshSections.Num(); SectionIdx++)
	{
		...
		if (Section.IsValid() && Section->IsCollisionEnabled())
		{
			...
			int32 NumFaces = Section->GetNumIndices(0) / 3;  //only use lod 0 for collision data
