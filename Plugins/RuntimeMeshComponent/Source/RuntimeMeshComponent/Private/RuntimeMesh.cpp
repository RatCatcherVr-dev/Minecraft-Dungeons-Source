...
bool URuntimeMesh::IsAsyncCollisionCookingComplete() const
{
	return AsyncBodySetupQueue.Num() == 0 && !bCollisionIsDirty;
}

void URuntimeMesh::ClearMeshDataBuffers()
{
	Data->ClearAllMeshSectionBuffers();
}
...
void URuntimeMesh::SetBasicBodySetupParameters(UBodySetup* Setup)
{
	...
	Setup->bDoubleSidedGeometry = false; //D11.SC no double sided?
