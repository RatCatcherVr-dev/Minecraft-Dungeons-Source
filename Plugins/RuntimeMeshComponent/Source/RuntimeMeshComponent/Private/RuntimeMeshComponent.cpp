...
void URuntimeMeshComponent::ClearMeshDataBuffers()
{
	URuntimeMesh* RuntimeMesh = GetRuntimeMesh();
	if (RuntimeMesh)
	{
		RuntimeMesh->ClearMeshDataBuffers();
	}
}

void URuntimeMeshComponent::ClearUserData() {
	UserData = nullptr;
}

void URuntimeMeshComponent::SetUserData(std::shared_ptr<void> userData) {
	UserData = std::move(userData);
}

void* URuntimeMeshComponent::GetUserData() {
	return UserData.get();
}
...
bool URuntimeMeshComponent::IsAsyncCollisionCookingPending() const
{
	...
	if (GetRuntimeMesh())
	{
		return !GetRuntimeMesh()->IsAsyncCollisionCookingComplete();
	}
	return false;
#endif
}
