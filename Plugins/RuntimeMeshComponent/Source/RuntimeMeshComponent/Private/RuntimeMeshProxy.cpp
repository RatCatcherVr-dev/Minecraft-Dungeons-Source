...
void FRuntimeMeshProxy::CreateSection_RenderThread(int32 SectionId, const FRuntimeMeshSectionCreationParamsPtr& SectionData)
{
	...
	TMap<int32, FRuntimeMeshSectionProxyPtr>& SectionsMap = Sections.FindOrAdd(SectionId);
	SectionsMap.FindOrAdd(SectionData->iLOD) = NewSection;
...
void FRuntimeMeshProxy::UpdateSection_RenderThread(int32 SectionId, const FRuntimeMeshSectionUpdateParamsPtr& SectionData)
{
	...
	{
		if (Sections[SectionId].Contains(SectionData->iLOD))
		{
			FRuntimeMeshSectionProxyPtr Section = Sections[SectionId][SectionData->iLOD];
