...
void FActiveGameplayCue::PostReplicatedAdd(const struct FActiveGameplayCueContainer &InArray)
{
	...
	{
		// If predicted ignore the add/remove
		//Mojang: In minimal replication mode we might as well do the add here and suppress the RPC all together.
		if(InArray.bMinimalReplication) InArray.Owner->InvokeGameplayCueEvent(GameplayCueTag, EGameplayCueEvent::OnActive, Parameters);
