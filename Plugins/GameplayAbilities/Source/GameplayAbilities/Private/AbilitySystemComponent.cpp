	...
	if (IsOwnerActorAuthoritative())
	{
		...
		// GE in his AGE array. Generate a server-side prediction key for him, which he will look for on the _Implementation function and ignore. (<--- Original Hack)
		{
			...
			if (ReplicationMode != EGameplayEffectReplicationMode::Minimal) {
				//Mojang: We are triggering our active in the container instead of the RPC, as this just seemed to be needless duplication of traffic.
		...
		if (!bWasInList && ReplicationMode != EGameplayEffectReplicationMode::Minimal)
		{
