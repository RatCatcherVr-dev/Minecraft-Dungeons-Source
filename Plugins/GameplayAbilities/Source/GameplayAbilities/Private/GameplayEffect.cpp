...
void FActiveGameplayEffectsContainer::OnStackCountChange(FActiveGameplayEffect& ActiveEffect, int32 OldStackCount, int32 NewStackCount, bool bChangedMagnitudes)
{
	MarkItemDirty(ActiveEffect);
	if (OldStackCount != NewStackCount || bChangedMagnitudes)
	{
...
FActiveGameplayEffect* FActiveGameplayEffectsContainer::FindStackableActiveGameplayEffect(const FGameplayEffectSpec& Spec)
{
	...
	if (StackingType != EGameplayEffectStackingType::None && (Spec.GetDuration() != UGameplayEffect::INSTANT_APPLICATION || Spec.Def->DurationMagnitude.GetMagnitudeCalculationType() == EGameplayEffectMagnitudeCalculation::CustomCalculationClass))
	{
...
FActiveGameplayEffect* FActiveGameplayEffectsContainer::ApplyGameplayEffectSpec(const FGameplayEffectSpec& Spec, FPredictionKey& InPredictionKey, bool& bFoundExistingStackableGE)
{
	...
	if (ExistingStackableGE)
	{
		...
		bChangedMagnitudes = ExistingSpec.SetByCallerNameMagnitudes.Num() != Spec.SetByCallerNameMagnitudes.Num();
		if (!bChangedMagnitudes) {
			for (auto it = ExistingSpec.SetByCallerNameMagnitudes.CreateConstIterator(); it; ++it) {
				for (auto itSpec = Spec.SetByCallerNameMagnitudes.CreateConstIterator(); itSpec; ++itSpec) {
					if (it->Key.IsEqual(itSpec->Key)) {
						if (it->Value != itSpec->Value) {
							bChangedMagnitudes = true;
						}
					}
				}
			}
	...
	if (ExistingStackableGE)
	{
		OnStackCountChange(*ExistingStackableGE, StartingStackCount, NewStackCount, bChangedMagnitudes);
...
void FActiveGameplayEffectsContainer::AddActiveGameplayEffectGrantedTagsAndModifiers(FActiveGameplayEffect& Effect, bool bInvokeGameplayCueEvents)
{
	...
	{	
		...
		{
			//Mojang: If we are using minimal replication we end up double adding Cues to UpdateTagMap if we invoke AddGameplayCue_MinimalReplication.
			if(ShouldUseMinimalReplication()) {
				//Mojang: Minimal replication uses scoped prediction key instead of the one passed, so we have to temporarily replace that here
				Swap(Effect.PredictionKey, Owner->ScopedPredictionKey);
...
bool FActiveGameplayEffectsContainer::InternalRemoveActiveGameplayEffect(int32 Idx, int32 StacksToRemove, bool bPrematureRemoval)
{
	...
	if (ensure(Idx < GetNumGameplayEffects()))
	{
		...
		if (!bIsNetAuthority && Effect.PredictionKey.IsLocalClientKey() && Effect.PredictionKey.WasReceived() == false)
		{
			...
			// Mojang: Or if we are using minimal replication mode (as no effect will ever come here from the server).
			if (HasReceivedEffectWithPredictedKey(Effect.PredictionKey) || Owner->ReplicationMode == EGameplayEffectReplicationMode::Minimal)
			{
...
void FActiveGameplayEffectsContainer::RemoveActiveGameplayEffectGrantedTagsAndModifiers(const FActiveGameplayEffect& Effect, bool bInvokeGameplayCueEvents)
{
	...
	if (!Owner->bSuppressGameplayCues)
	{
		//Mojang: Updated with fix according to epic as per https://udn.unrealengine.com/questions/551424/view.html
		for (const FGameplayEffectCue& Cue : Effect.Spec.Def->GameplayCues)
		{
			Owner->UpdateTagMap(Cue.GameplayCueTags, -1);
...
void FActiveGameplayEffectsContainer::ModifyActiveEffectStartTime(FActiveGameplayEffectHandle Handle, float StartTimeDiff)
{
	...
	if (FActiveGameplayEffect* Effect = GetActiveGameplayEffect(Handle))
	{
		...
		//Mojang: This will remove the effect if expired, causing the above function to dereference now freed memory.
		//To mitigate this, we move this call to the bottom.
